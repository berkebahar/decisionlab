...
export default function CalculatorWorkspace() {
  ...
  if (goalId && snapshot === undefined) return <Skeleton label="Opening your goal…" />;
  return <>
    {error ? <div className="load-notice" role="status"><p>{error} The example below is ready to explore.</p><Link className="text-link" href="/dashboard">Back to your dashboard →</Link></div> : goal && <div className="load-notice"><p><Icon name="check" size={17} /> Exploring <strong>{goal.name}</strong>. Changes here apply to this comparison. Update your saved goal in the dashboard.</p>{goal.weeklyContribution === 0 && <p>Add weekly savings below to calculate a timeline for this paused goal.</p>}</div>}
