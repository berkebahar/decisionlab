...
export default function GoalDashboard() {
  ...
  return <section className="goals-section" aria-labelledby="goals-heading">
    ...
    {isDemo && <div className={styles.demoBanner} role="status"><div><strong><Icon name="spark" size={17} /> You’re exploring sample savings goals</strong><p>These example goals are a preview. Your personal goals and saved decisions are unchanged.</p></div><button ref={exitPreviewButton} type="button" className="button-outline" onClick={() => togglePreview(false)}>Exit preview <Icon name="close" size={16} /></button></div>}
    ...
    {snapshot === undefined && <Skeleton label="Preparing your savings goals" />}
    {snapshot !== undefined && !error && goals.length === 0 && !editor && !isDemo && <div className={`empty-state ${styles.emptyState}`}>
      ...
      <div className={styles.emptyActions}><button className="button-primary" type="button" onClick={(event) => createGoal(event.currentTarget)}>Create your first goal <Icon name="plus" size={18} /></button><button ref={previewButton} className="button-outline" type="button" onClick={() => togglePreview(true)}>Explore sample savings goals <Icon name="arrow" size={17} /></button></div>
