...
function QueueCard({ record, today, onRemove }: { record: ProductRecord; today: string; onRemove: () => void }) {
  ...
  return <article className="queue-card"><header><div><p className="eyebrow">{statusLabels[record.status]} · {record.analysis.condition}</p><h2>{record.analysis.name}</h2><p>Estimated ownership cost: <strong>{formatMoney(r.net, record.analysis.currency)}</strong></p></div>{record.reconsiderOn && <p className="queue-date">{record.reconsiderOn <= today ? "Ready to reconsider" : "Reconsider on"}<time dateTime={record.reconsiderOn}>{record.reconsiderOn}</time></p>}</header>
...
export default function QueueWorkspace() {
  ...
  return <div><p className="product-notice">Local reminders only: dates appear when you return to DecisionLab. No notifications or emails are sent. These records stay on this browser/device.</p><div className="product-toolbar"><Link href="/analyze" className="button-primary">Analyze a product</Link><div className="product-field"><label htmlFor="queue-filter">Show</label><select id="queue-filter" value={filter} onChange={event => setFilter(event.target.value)}><option value="active">Under consideration</option><option value="all">All analyses and decisions</option><option value="skipped">Skipped</option><option value="bought">Bought</option></select></div><Link className="text-link" href="/purchases">Purchase library →</Link></div><p role="status" className={error ? "product-error" : "product-status"}>{error || message}</p>
