...
function ReviewEditor({ record }: { record: ProductRecord }) {
  ...
  return <form className="purchase-review-form" onSubmit={event => { event.preventDefault();
    ...
    try { const result = saveReview(window.localStorage, record.id, review, version); setVersion(result.updatedAt); notifyStorageChange(); setMessage("Review saved. Your original prediction is unchanged."); }
...
export default function PurchaseLibrary() {
  ...
  if (loading) return <Skeleton label="Loading your purchase library" />;
