...
function AnalysisEditor({ record, duplicate = false, demo }: { record?: ProductRecord; duplicate?: boolean; demo?: ProductAnalysis }) {
  ...
  return analysis && <div ref={result} className="analysis-result"><TrueCostReceipt analysis={analysis} demo={demoActive} /><aside className="receipt-next"><p className="eyebrow">Step 5 of 5 · Receipt</p><h2>Your next step</h2><p>Save this receipt to reconsider later, or refine your assumptions.</p><div className="product-actions">{demoActive ? <button type="button" className="button-outline" onClick={() => setDemoActive(false)}>Use this example as my starting point</button> : <button type="button" className="button-primary" onClick={save} disabled={busy || products.saving || products.loading || !!products.error || products.mode === "pending"}>{busy ? "Saving…" : saved ? "Save updated receipt" : "Save receipt to queue"}</button>}<button type="button" className="button-outline" onClick={() => setEditing(true)}>Edit assumptions</button></div><p role="status" className="product-status">{message}</p>{saved && <><Link className="text-link" href={`/compare?ids=${encodeURIComponent(saved.id)}`}>Compare this product →</Link><Link className="text-link" href="/queue">Open decision queue →</Link></>}<p className="product-helper">{products.mode === "cloud" ? "Saved receipts are private to your account." : "Records stay in this browser. Export a local backup from the queue."}</p>{record?.purchaseEstimate && <p className="product-helper">The original estimate captured when marked bought remains unchanged for post-purchase reviews.</p>}</aside></div>;
}
