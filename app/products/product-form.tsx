...
export default function ProductForm({ initial, onComplete, onCancel }: { initial?: ProductAnalysis; onComplete: (analysis: ProductAnalysis) => void; onCancel?: () => void }) {
  ...
  return <form ref={form} className="product-form" onInvalidCapture={event => revealInvalidField(event.target as HTMLElement)} onSubmit={event => { event.preventDefault(); if (step < 3) changeStep(step + 1); else submit(); }}>
    ...
    <div className="product-form-heading"><p className="eyebrow">Step {step + 1} of 5</p><h2 ref={heading} id={`${prefix}-stage-heading`} tabIndex={-1}>{steps[step]}</h2><p>Use your own estimates. Blank optional costs remain unknown; enter 0 only when a cost does not apply. All amounts use {fields.currency}.</p></div>
    ...
    {step === 3 && <>
      <section className="score-settings"><label className="product-check"><input type="checkbox" checked={fields.hasScore} onChange={event => update("hasScore", event.target.checked)} />Include a user-specific product score</label><p className="product-helper">Optional. Choose your limits to measure personal fit. This is not a buy probability or an affordability check. Each selected factor has equal weight; leave a limit blank to exclude it.</p>
        {fields.hasScore && <div className="product-fields">
          ...
          <p className="product-helper product-wide">A score needs the cost estimates used by your selected factors. Unknown costs remain unknown; enter 0 only when they do not apply. Selecting multiple cost factors gives cost more influence.</p>
        </div>}
      </section>
    ...
    <p className="product-helper">{step === 3 ? "Next: your receipt, with every assumption visible." : "Optional sections can be left blank. Your receipt will list missing information."}</p><div className="product-actions">{step > 0 && <button className="button-outline" type="button" onClick={() => changeStep(step - 1)}>Back</button>}<button className="button-primary" type="submit">{step === 3 ? "Generate True Cost Receipt" : "Continue"}</button>{onCancel && <button type="button" className="button-quiet" onClick={onCancel}>Cancel editing</button>}</div>
  </form>;
}
